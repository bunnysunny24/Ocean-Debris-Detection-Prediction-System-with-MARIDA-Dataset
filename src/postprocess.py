...
from configs.config import MIN_DEBRIS_PIXELS, CLASS_NAMES, OUTPUTS_DIR
...
def vectorise_patch(pred_path: str) -> list:
    ...
    # Saved file stores (pred_mask + 1): DN 1 = debris, DN 2 = not-debris
    ...
    if HAS_CRF and img is not None:
        ...
        mask_probs[0] = (data == 1).astype(np.float32)   # debris  = DN 1
        mask_probs[1] = (data == 2).astype(np.float32)   # not-debris = DN 2
        crf_mask = crf_refine(img, mask_probs, n_classes=2, crf_iters=5)
        binary = (crf_mask == 0).astype(np.uint8)
    else:
        binary = (data == 1).astype(np.uint8)  # DN 1 = debris
...
if __name__ == "__main__":
    ...
    p.add_argument("--pred_dir", default=os.path.join(OUTPUTS_DIR, "predicted_test"))
    p.add_argument("--out_dir",  default=os.path.join(OUTPUTS_DIR, "geospatial"))
    args = p.parse_args()
