...
def main(args):
    ...
    # Train in chunks so we can show progress with tqdm. warm_start=True
    # allows increasing `n_estimators` across successive fit calls.
    total_trees = 300
    chunk = 10
    # Compute class weights from the training labels to avoid sklearn warnings
    ...
    log.info("Training Random Forest ...")
    pbar = tqdm(total=total_trees, desc="RF trees", unit="trees")
    grown = 0
    while grown < total_trees:
        next_n = min(grown + chunk, total_trees)
        rf.n_estimators = next_n
        rf.fit(X_train, y_train, sample_weight=conf_w)
        # update progress bar by number of newly grown trees
        pbar.update(next_n - grown)
        grown = next_n
    pbar.close()
    # Rebuild a final RF with oob_score=True to get a reliable OOB estimate.
    ...
    final_rf.fit(X_train, y_train, sample_weight=conf_w)
    rf = final_rf
    try:
        log.info(f"OOB score: {rf.oob_score_:.4f}")
    except Exception:
        log.info("OOB score unavailable")

    # -- Evaluate --
    y_pred = rf.predict(X_test)
    # Determine number of classes from the data (labels were converted to 0-indexed)
    n_classes = int(max(y_train.max() if len(y_train) else 0, y_test.max() if len(y_test) else 0) + 1)
    # Build class name list: prefer names from CONFIG `CLASS_NAMES`, otherwise fallback to generic labels
    class_names = [CLASS_NAMES.get(i, f'Class {i+1}') for i in range(n_classes)]
